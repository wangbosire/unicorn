...
定义 M4「流转与通知闭环」在当前仓库实现下的验收范围、自动化回归口径与演示脚本，与 [一期里程碑总览](./milestones-overview.md)、[V1 执行计划](./v1-exec-plan.md) 对齐；在 M3 与评论治理专项完成后，作为 **后台运营视角的流转 / 消息 / 总览阶段** 验收基线。

## 当前状态

- 状态：`active`
- 上位计划：[V1 执行计划](./v1-exec-plan.md)
- 前置里程碑：M3（见 [M3 验收清单](./m3-acceptance-checklist.md)）
- 当前实现快照：后台仪表盘、通知中心、转让记录与会员正式 Bearer 鉴权均已接入真实数据或真实鉴权路径，待统一按本清单完成自动化与演示收口
...
- **转让体系**：验收后台转让记录列表、方式/状态筛选、去向展示与演示种子数据；当前不把会员侧完整「发起转让 -> 接收转让 -> 持有权变更」闭环视为本清单必过项。
...
- 会员正式登录与当前身份确认：微信小程序登录、`auth/me`、冻结账号拦截。
- 与上述能力直接相关的 API / Admin / Miniapp 自动化回归。

## 范围外事项

- 会员端完整转让发起、接收、撤销、到最终持有权迁移闭环。
