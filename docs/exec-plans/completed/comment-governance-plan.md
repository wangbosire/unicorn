...
本文档用于保留评论治理专项的范围、实施顺序与落地结论，作为 **评论治理专项已归档** 后的基线与复盘依据。

## 当前状态

- 状态：`completed`（评论治理专项已于 2026-05-18 完成归档；本次归档基于后端、后台与公开展示/会员交互链路收口结果）
- 执行顺位：**已完成**
- 上位计划：[V1 执行计划](../active/v1-exec-plan.md)
- 关联里程碑：
  - [M3 验收清单（已归档）](../completed/m3-acceptance-checklist.md)
  - [里程碑总览](../active/milestones-overview.md)
- 背景说明：评论 Prisma 模型、三类 API、后台真实治理页面、公开页评论展示、会员评论/回复交互与专项回归入口均已落地，当前文档转为归档保留。
...
- `apps/admin` 评论列表 / 评论审核页已完成真实接口对接
- 仓库根评论治理专项回归入口已统一为 `pnpm test:comments`
- 小程序公开展示页与会员评论 / 回复交互已完成最小可用闭环
...
- 2026-05-18：按代码现状回写计划口径：评论 Prisma 模型、三类 API、种子数据与后端测试已落地，并在同日完成 Admin 页面对接真实接口、专项回归与归档收口。
