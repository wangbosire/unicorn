...
本文档用于保留 M3「审核治理闭环」的验收范围、条件与演示口径，与 [一期里程碑总览](../active/milestones-overview.md)、[V1 执行计划](../active/v1-exec-plan.md) 对齐，作为 **M3 已归档** 后的阶段验收基线与复盘依据。

## 当前状态

- 状态：`completed`（M3「审核治理闭环」已于 2026-05-18 完成归档；本次归档基于已合并实现、自动化回归与演示脚本收口结果）
- 上位计划：[V1 执行计划](../active/v1-exec-plan.md)
- 前置里程碑：M2（见 [M2 验收清单（已归档）](../completed/m2-acceptance-checklist.md)）
...
- **机审通过后默认口径**：保持 M2 既有策略，默认仍为「同步机审通过即可公开」；`CONTENT_MANUAL_GATE_AFTER_MACHINE` 继续保留为可切换开关，用于本地联调、灰度验证或后续策略切换，不作为当前 M3 的默认发布门槛。
- **M3 核心闭环**：以 **人工复核队列 -> 人工通过/驳回 -> 审核历史 -> 下架公开 -> 会员冻结/解冻拦截** 作为当前阶段必须走通的治理主链路。
- **评论审核处理方式**：由于后台评论审核当前仍为占位 mock，且尚无对应 `admin-api` 评论治理接口，**本轮 M3 不纳入评论审核实现与验收**；评论治理延后到 M4 或后续专项计划，再以独立执行计划收口。
...
- 评论审核与评论治理接口落地（当前后台仍为 mock 占位，后续单列计划收口）。
- 小程序内完整「社交评论」产品。
...
- 固化后台默认工作台口径：`PENDING_MANUAL` 队列优先、人工通过 / 驳回错误码清晰、操作后列表与历史同步刷新。
- 验收结果：运营可在后台连续处理至少一条待人工记录，并从历史中看到完整状态迁移。

### 任务 2：下架公开与公开兜底补齐

- 继续稳定 `TAKEDOWN` 的运营入口、公开 410 返回、小程序中文提示与前端状态展示。
- 验收结果：同一藏品可完成「已公开 -> 下架 -> 公开接口 410 -> 小程序提示下架」闭环。

### 任务 3：会员治理最小闭环

- 以会员列表、搜索 / 状态筛选、冻结 / 解冻及会员端鉴权拦截为最小交付面。
- 验收结果：冻结会员后访问 `member-api` 返回 `MEMBER_ACCOUNT_FROZEN`，解冻后恢复正常。
