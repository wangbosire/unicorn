...
describe('SearchProvider and CommandMenu', () => {
  ...
  it('navigates to a top-level route and closes the palette when a nav item is selected', async () => {
    ...
    await userEvent.click(screen.getByText('系列管理'))

    expect(mocks.navigate).toHaveBeenCalledWith({ to: '/issuance/series' })
    await expect
      .element(screen.getByPlaceholder(COMMAND_MENU_PLACEHOLDER))
      .not.toBeInTheDocument()
  })

  it('navigates when another sidebar-linked item is selected', async () => {
    const screen = await renderWithSearchProvider()
    const { getByPlaceholder, getByText } = screen

    await openCommandPalette(screen)

    await userEvent.click(getByText('内容复核'))

    expect(mocks.navigate).toHaveBeenCalledWith({ to: '/collections/reviews' })
    await expect
      .element(getByPlaceholder(COMMAND_MENU_PLACEHOLDER))
      .not.toBeInTheDocument()
  })
